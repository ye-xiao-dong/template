package com.yd.springbootdemo.utils.map;
...
public class MapUtil {
    ...
    public static <K, V> Map<K, V> filter(Map<K, V> map, Editor<Entry<K, V>> editor) {
        ...
        final Map<K, V> map2 = ReflectUtil.newInstance(map.getClass());
        Entry<K, V> modified;
        for (Entry<K, V> entry : map.entrySet()) {
    ...
    public static <K, V> Map<K, V> getAny(Map<K, V> map, final K... keys) {
        return filter(map, (Predicate<Entry<K, V>>) entry -> ArrayUtil.contains(keys, entry.getKey()));
    }
     ...
     * @since 4.5.12
     */
    public static <T> T get(Map<?, ?> map, Object key, TypeRef<T> type) {
        return null == map ? null : Convert.convert(type, map.get(key));
    }
