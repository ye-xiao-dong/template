		...
		try {
			if (byte.class == this.targetType) {
				if (value instanceof Number) {
					...
					return BoolUtil.toByte((Boolean)value);
				}
				...
				if (value instanceof Number) {
					...
					return BoolUtil.toShort((Boolean)value);
				}
				final String valueStr = convertToStr(value);
				if (StrUtil.isBlank(valueStr)) {
					return 0;
				...
				if (value instanceof Number) {
					...
					return BoolUtil.toInt((Boolean)value);
				}
				final String valueStr = convertToStr(value);
				if (StrUtil.isBlank(valueStr)) {
					return 0;
				}
				return NumUtil.parseInt(valueStr);
				
			} else if (long.class == this.targetType) {
				if (value instanceof Number) {
					return ((Number) value).longValue();
				} else if(value instanceof Boolean) {
					return BoolUtil.toLong((Boolean)value);
				}
				final String valueStr = convertToStr(value);
				if (StrUtil.isBlank(valueStr)) {
					return 0;
				}
				return NumUtil.parseLong(valueStr);
				
			} else if (float.class == this.targetType) {
				if (value instanceof Number) {
					return ((Number) value).floatValue();
				} else if(value instanceof Boolean) {
					return BoolUtil.toFloat((Boolean)value);
				}
				final String valueStr = convertToStr(value);
				if (StrUtil.isBlank(valueStr)) {
					return 0;
				...
				if (value instanceof Number) {
					...
					return BoolUtil.toDouble((Boolean)value);
				}
				final String valueStr = convertToStr(value);
				if (StrUtil.isBlank(valueStr)) {
					return 0;
				...
				if(value instanceof Character){
					...
					return BoolUtil.toChar((Boolean)value);
				}
				final String valueStr = convertToStr(value);
				if (StrUtil.isBlank(valueStr)) {
					return 0;
				...
				return BoolUtil.toBoolean(valueStr);
			}
		} catch (Exception e) {
			// Ignore Exception
		}
