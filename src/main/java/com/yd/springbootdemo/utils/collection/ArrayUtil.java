...
public class ArrayUtil {
	...
	public static <T extends CharSequence> T[] removeEmpty(T[] array) {
		return filter(array, new Predicate<T>() {
			@Override
	...
	public static <T extends CharSequence> T[] removeBlank(T[] array) {
		return filter(array, (Predicate<T>) t -> false == StrUtil.isBlank(t));
	}
	...
	public static <T extends Comparable<? super T>> T min(T[] numberArray) {
		...
		for (int i = 0; i < numberArray.length; i++) {
			if (ObjUtil.compare(min, numberArray[i]) > 0) {
				min = numberArray[i];
			}
		}
		return min;
	...
	public static <T extends Comparable<? super T>> T max(T[] numberArray) {
		...
		T max = numberArray[0];
		for (int i = 0; i < numberArray.length; i++) {
			if (ObjUtil.compare(max, numberArray[i]) < 0) {
				max = numberArray[i];
			}
		}
		return max;
